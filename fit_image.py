...
for _file in files:
    ...
    for index_fitting in range(len(smpl_prediction)):
        segmentation = cv2.imread(path_segmentation, 0) # (h, w)
        ...
        topleft = smpl_prediction[index_fitting]['bbox_top_left']
        scale_ratio = smpl_prediction[index_fitting]['bbox_scale_ratio']
        pose = torch.FloatTensor(smpl_prediction[index_fitting]['pred_body_pose']) # (1, 72)
        beta = torch.FloatTensor(smpl_prediction[index_fitting]['pred_betas']) # (1, 10)
